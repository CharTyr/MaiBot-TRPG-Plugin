...
**加点流程**：
```
1. /trpg join 李明           # 加入并创建角色
2. /trpg pc add 力量 4       # 力量 8→12，消耗4点
3. /trpg pc add 敏捷 3       # 敏捷 8→11，消耗3点
4. /trpg pc add 智力 5       # 智力 8→13，消耗5点
5. /trpg pc lock             # 锁定角色，开始冒险
```

**加点命令**：
| 命令 | 说明 |
|------|------|
| `/trpg pc add 属性 点数` | 增加属性（消耗点数） |
| `/trpg pc sub 属性 点数` | 减少属性（返还点数） |
| `/trpg pc reset` | 重置所有加点 |
| `/trpg pc lock` | 锁定角色 |
| `/trpg pc show` | 查看角色卡和剩余点数 |
...
```toml
[player]
free_points = 30      # 初始加点点数
base_attribute = 8    # 基础属性值
max_attribute = 18    # 属性上限
min_attribute = 3     # 属性下限
```
...
当模组有 2 人以上参与时，插件会自动启用**等待所有玩家行动**模式：

**行动流程**：
1. 玩家 A 发送行动 → 收到确认，开始等待其他玩家
2. 系统显示等待状态 `⏳ 等待其他玩家行动... (1/3)`
3. 玩家 B 发送行动 → 显示 `✅ 玩家B 已行动 (2/3)`
4. 每 20 秒提醒未行动的玩家
5. 所有玩家行动后 → 立即处理
6. 或等待 60 秒超时 → 处理已收集的行动（跳过未行动玩家）

**等待提示示例**：
```
⏳ 等待其他玩家行动... (1/3)
💡 最长等待 60 秒，或所有玩家行动后立即处理
...
action_collect_window = 60.0   # 最大等待时间（秒）
reminder_interval = 20.0       # 提醒间隔（秒）
process_when_all_ready = true  # 所有人行动后立即处理
```
...
DM 在叙述中可以使用特殊标签来触发玩家状态的自动变化，系统会解析并应用这些变化：

**支持的状态标签**：
| 标签格式 | 示例 | 说明 |
|---------|------|------|
| `[HP ±数值]` | `[HP -5]` `[HP +10]` | 生命值变化 |
| `[MP ±数值]` | `[MP -3]` `[MP +5]` | 魔力值变化 |
| `[获得 物品]` | `[获得 铜钥匙]` `[获得 金币 x10]` | 获得物品 |
| `[失去 物品]` | `[失去 钥匙]` `[消耗 药水 x2]` | 失去/消耗物品 |
| `[属性 ±数值]` | `[力量 +2]` `[敏捷 -1]` | 属性变化 |
| `[移动到 位置]` | `[移动到 图书馆]` | 位置变化 |

**DM 响应示例**：
```
你被陷阱击中 [HP -5]，但成功找到了一把钥匙 [获得 铜钥匙]。
经过艰苦的战斗，你击败了敌人 [获得 经验值 x50]，但消耗了不少体力 [MP -3]。
```
