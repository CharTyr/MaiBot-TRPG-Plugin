...
插件会从 DM 回复中解析「状态变化标签」并自动应用到角色卡/会话中。

- 单人回合：允许直接写 `"[HP -5]"` / `"[获得 钥匙]"` 等（默认作用于当前玩家）
- 多人回合（批量行动）：**必须带 uid**，避免把同一条变化重复应用到所有玩家，例如：
  - `[uid:355864328 HP -5]`
  - `[uid:355864328 获得 钥匙 x1]`
  - `[移动到 图书馆]`（会话公共变化，不需要 uid）
...
1. **战斗系统增强** - 目前以叙事为主，可逐步补齐战斗回合/先攻/伤害等规则
2. **WebUI 适配** - 需要了解 MaiBot WebUI 系统后实现更友好的界面与状态展示

### 中优先级

3. **更多预设模组** - 扩充不同题材与难度的开箱即用模组
4. **多语言支持**

### 低优先级

5. **更细的规则系统** - 例如 CoC/DnD5e 的专用检定与角色卡模板
